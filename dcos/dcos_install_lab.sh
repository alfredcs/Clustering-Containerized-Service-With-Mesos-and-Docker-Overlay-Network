...
["dcos-config--setup_05155cc7e68aa6c36f41ec5d47f9703c07075bee", "dcos-metadata--setup_05155cc7e68aa6c36f41ec5d47f9703c07075bee"]
EOF
...
StandardOutput=journal+console
StandardError=journal+console
ExecStartPre=/usr/bin/curl -fLsSv --retry 20 -Y 100000 -y 60 -o /tmp/bootstrap.tar.xz http://10.11.4.32:8088/bootstrap/${BOOTSTRAP_ID}.bootstrap.tar.xz
...
function pre_requisite() {
    yum -y update; yum -y upgrade;yum -y install ntp
    setenforce 0
...
[Service]
ExecStart=
ExecStart=/usr/bin/docker daemon --storage-driver=overlay --storage-opt dm.no_warn_on_loop_devices=true -H fd:// -H tcp://0.0.0.0:4243  --insecure-registry dcos1-bootstrap.vms.crd.ge.com:5000
EOF
    tee /etc/systemd/system/docker.service.d/http-ptoxy.conf <<-'EOF'
[Service]
Environment="HTTP_PROXY=http://sjc1intproxy01.crd.ge.com:8080/" "HTTPS_PROXY=http://sjc1intproxy01.crd.ge.com:8080/" "NO_PROXY=localhost,127.0.0.1,dcos1-bootstrap.vms.crd.ge.com"
EOF
...
EOF
   ...
   # Add nfs
   [[ `grep '/opt/nfs1' /etc/fstab | grep -v ^#|wc -l` -lt 1 ]] && echo "192.168.12.21:/opt/shares	/opt/nfs1			  nfs rsize=8192,wsize=8192,timeo=14,intr" >> /etc/fstab
   [[ `grep '/opt/nfs2' /etc/fstab | grep -v ^#|wc -l` -lt 1 ]] && echo "192.168.12.25:/opt/shares	/var/lib/registry		  nfs rsize=8192,wsize=8192,timeo=14,intr" >> /etc/fstab
...
EOM
	...
	#Alfred allows idempotence 
        #exit 1
	print_status 0 "${NORMAL}(existing installation)"
    else
...
function check_all() {
    ...
    print_status $RC
    (( OVERALL_RC += $RC ))
    #for service in       "80 mesos-ui"       "53 mesos-dns"       "15055 dcos-history"       "5050 mesos-master"       "2181 zookeeper"       "8080 marathon"       "3888 zookeeper"       "8181 exhibito
r"       "8123 mesos-dns"
    for service in       "80 mesos-ui"       "15055 dcos-history"       "5050 mesos-master"       "2181 zookeeper"       "8080 marathon"       "3888 zookeeper"       "8181 exhibitor"       "8123 mesos-d
ns"
    do
      check_service $service
    done
    echo -e "check2 ${OVERALL_RC}"
...
function main()
{
    ...
    #
cat <<'EOF' >> /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem
-----BEGIN CERTIFICATE-----
